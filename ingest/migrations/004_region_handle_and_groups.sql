...
--
-- The topic->region derivation below is the canonical SQL form. It is kept byte-for-byte
-- identical to regionSql('topic') in meshexplorer/src/lib/regions.ts; the parity script
-- (meshexplorer/scripts/region-parity.ts) greps every occurrence here and fails on drift.
-- Canonical (REGION-DERIVATION-CANONICAL):
...
ALTER TABLE meshcore_packets
  ...
  -- REGION-DERIVATION-CANONICAL-END
  COMMENT 'Derived IATA region from base topic (read-time ALIAS, mirrors regionSql in src/lib/regions.ts)';
-- +goose StatementEnd

-- 2. Region groups: named sets of IATA codes. Single source of truth shared by the app and
--    Grafana. Mirror of REGION_GROUPS in meshexplorer/src/lib/regionGroups.ts (parity-enforced).
CREATE TABLE IF NOT EXISTS region_groups (
    group_code  LowCardinality(String) COMMENT 'Group key, e.g. PNW',
...
COMMENT 'Region-group membership. Mirror of REGION_GROUPS in src/lib/regionGroups.ts; parity script enforces.';

INSERT INTO region_groups (group_code, group_name, region_code) VALUES
  ('PNW','Pacific Northwest','SEA'),('PNW','Pacific Northwest','PDX'),('PNW','Pacific Northwest','YVR'),
  ('PNW','Pacific Northwest','BLI'),('PNW','Pacific Northwest','GEG'),('PNW','Pacific Northwest','OLM'),
  ('PNW','Pacific Northwest','PAE'),('PNW','Pacific Northwest','EAT'),('PNW','Pacific Northwest','RLD'),
  ('PNW','Pacific Northwest','PUW'),('PNW','Pacific Northwest','KEH'),('PNW','Pacific Northwest','KLS'),
  ('PNW','Pacific Northwest','TDO'),('PNW','Pacific Northwest','EUG'),('PNW','Pacific Northwest','CVO'),
  ('PNW','Pacific Northwest','MFR'),('PNW','Pacific Northwest','TTD'),('PNW','Pacific Northwest','SLE'),
  ('PNW','Pacific Northwest','MMV'),('PNW','Pacific Northwest','ONP'),('PNW','Pacific Northwest','LMT'),
  ('PNW','Pacific Northwest','OTH'),('PNW','Pacific Northwest','YCD'),('PNW','Pacific Northwest','YYJ'),
  ('PNW','Pacific Northwest','YKA'),
  ('CAL','California','LAX'),('CAL','California','SFO'),('CAL','California','SJC'),
  ('CAL','California','OAK'),('CAL','California','SMF'),('CAL','California','SAN'),('CAL','California','OXR'),
  ('DEU','Germany','BER'),('DEU','Germany','HAM'),('DEU','Germany','MUC'),('DEU','Germany','FRA'),
  ('DEU','Germany','CGN'),('DEU','Germany','DUS'),('DEU','Germany','STR'),('DEU','Germany','LEJ'),
  ('DEU','Germany','BWE'),('DEU','Germany','HAJ'),('DEU','Germany','NUE'),
  ('POL','Poland','WAW'),('POL','Poland','KRK'),('POL','Poland','WRO'),('POL','Poland','POZ'),
  ('POL','Poland','GDN'),('POL','Poland','KTW'),('POL','Poland','LCJ'),('POL','Poland','BZG');
